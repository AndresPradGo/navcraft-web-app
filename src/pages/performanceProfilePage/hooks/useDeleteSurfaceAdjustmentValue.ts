...
const useDeleteSurfaceAdjustmentValue = (profileId: number, isTakeoff: boolean) => {
  ...
  return useMutation<string, APIClientError, number, TakeoffLandingContext>({
    mutationFn: surfaceId => {
      ...
      const newData = (previousData ? {
        ...previousData, 
        percent_increase_runway_surfaces: previousData.percent_increase_runway_surfaces.filter(
    ...
    onMutate: surfaceId => {
      ...
      queryClient.setQueryData<TakeoffLandingDataFromAPI>(
        ...
        currentData => {
          return (currentData ? {
              ...currentData,
              percent_increase_runway_surfaces: currentData.percent_increase_runway_surfaces.filter(
                item => item.surface_id !== surfaceId
              )
          }: undefined)
      })
