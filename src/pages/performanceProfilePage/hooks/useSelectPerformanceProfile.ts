...
const apiClient = new APIClient<number, PerformanceProfileBaseData>(
  '/aircraft/performance-profile/make-preferred',
);
...
const useSelectPerformanceProfile = (aircraftId: number) => {
  ...
  >({
    mutationFn: (id) =>
      apiClient.editAndPreProcess<CompletePerformanceProfileDataFromAPI>(
        id,
        (dataFromApi) => ({
          id: dataFromApi.id,
