...
const apiClient = new APIClient<AddProfileData, PerformanceProfileBaseData>(
...
const useEditPerformanceProfile = (aircraftId: number) => {
  ...
  >({
                  ...
                  profile.id === newData.id
                    ? {
                        ...
                        performance_profile_name:
                          newData.performance_profile_name,
    ...
    onSuccess: (savedData) => {
      toast.success(
        `${savedData.performance_profile_name}, has been updated successfully.`,
        {
          position: 'top-center',
          autoClose: 10000,
