...
import apiClient, {WeightAndBalanceProfileType} from '../../../services/weightBalanceProfileClient'
  ...
  const queryClient = useQueryClient()
    ...
    WeightBalanceDataContext>({
      mutationFn: data => {
        const formatedData ={
          name: data.name,
          limits: data.limits.map((limit,idx) => ({...limit, sequence: idx + 1}))
        }
    ...
    onMutate: newData => {
      ...
      queryClient.setQueryData<WeightAndBalanceDataFromAPI>(['AircraftWeightBalanceData', profileId], currentData => {
        if (newData.id === 0) {
          return (currentData ? {
            ...
            weight_balance_profiles: [
              ...currentData.weight_balance_profiles, 
              {
                id: 0,
                name: newData.name,
                limits : newData.limits.map((limit, idx) => ({...limit, sequence: idx + 1, id: idx + 1})),
          ...
          return (currentData ? {
            ...currentData,
            weight_balance_profiles: currentData.weight_balance_profiles.map((profile) => {
              if (profile.id === newData.id) return({
                ...profile,
                name: newData.name,
                limits : newData.limits.map((limit, idx) => ({...limit, sequence: idx + 1, id: idx + 1})),
                last_updated_utc: getUTCNowString()
    ...
    onSuccess: (savedData, newData) => {
      ...
      queryClient.invalidateQueries<WeightAndBalanceDataFromAPI>(['AircraftWeightBalanceData', profileId])
    },
