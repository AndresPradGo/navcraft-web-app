...
const useEditClimbData = (profileId: number) => {
    ...
    return useMutation<string, APIClientError, ClimbAdjustmentValuesFromForm, ClimbContext>({
        mutationFn: data => {
            return (apiClient.editAndGetOther<string>(data, `/climb-adjustments/${profileId}`))
        },
        onMutate: newData => {
            const previousData = queryClient.getQueryData<ClimbPerformanceDataFromAPI>(
            ...
            queryClient.setQueryData<ClimbPerformanceDataFromAPI>(
                ...
                currentData => {
                    return (currentData ? {
                        ...
                        percent_increase_climb_temperature_c: newData.percent_increase_climb_temperature_c
                    }: undefined)
                }
            )
            return {previousData}
        ...
        onSuccess: () => {
            toast.success(`Climb performance adjustment values have been updated successfully.`, {
                position: "top-center",
                autoClose: 10000,
