...
import apiClient, {FlightSummaryDataFromApi, AddFlightData} from '../../services/flightsClient'
...
const useAddFlight = () => {
  ...
  return useMutation<FlightSummaryDataFromApi, APIClientError, AddFlightData, FlightContext>({
    mutationFn: data => (apiClient.post(data)),
    onMutate: newData => {
        const previousData = queryClient.getQueryData<FlightSummaryDataFromApi[]>(['flights', 'all']) 
        queryClient.setQueryData<FlightSummaryDataFromApi[]>(['flights', 'all'], currentData => {
            const flightToAdd = {
                id: 0,
