...
const useDeletePassenger = () => {
    ...
    return useMutation<string, APIClientError, DeletePassegerData, DeletePassengerContext>({
        mutationFn: (passenger: DeletePassegerData) => apiClient.delete(`/${passenger.id}`),
        onMutate: (passengerData) => {
            ...
            queryClient.setQueryData<PassengerDataInCache[]>(
                ['passengers'], 
                currentData => (
                    currentData ? currentData.filter(item => item.id !== passengerData.id) : []
                )
            )
            return { previusData }
        },
