...
const useEditUser = () => {
  ...
  >({
    ...
    onMutate: (newData) => {
      const previusData = queryClient.getQueryData<UserDataFromAPI[]>([
        'users',
      ]);
      queryClient.setQueryData<UserDataFromAPI[]>(['users'], (currentData) => {
        return (
          currentData?.map((item) => {
            if (item.id === newData.id)
              return {
                ...item,
                is_admin: newData.make_admin,
                is_active: newData.activate,
              };
            return item;
          }) || []
        );
    ...
    onSuccess: (savedData, newData) => {
      ...
      queryClient.setQueryData<UserDataFromAPI[]>(['users'], (currentData) => {
        return (
          currentData?.map((item) => {
            if (item.id === newData.id) return { ...savedData };
            return item;
          }) || []
        );
      });
