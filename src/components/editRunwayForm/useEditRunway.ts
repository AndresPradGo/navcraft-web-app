...
const useEditRunway = (fromAerodrome: boolean) => {
  ...
  return useMutation<
    RunwayData,
    APIClientError,
    EditRunwayDataWithIdAndSurface,
    AerodromeContext
  >({
    mutationFn: (data) => {
      const runwayData = {
    ...
    onMutate: (newData) => {
                      ...
                      if (item.id === newData.id) {
                        return {
                          ...
                          intersection_departure_length_ft:
                            newData.intersection_departure_length_ft,
      ...
      const aerodrome =
        previousRunwaysData?.find(
          (item) => item.aerodrome_id === newData.aerodrome_id,
        )?.aerodrome || '';
      queryClient.setQueryData<RunwayData[]>(['runways'], (currentData) => {
        if (newData.id !== 0) {
        ...
        queryClient.setQueryData<AerodromeDataFromAPI>(
          ...
          (aerodrome) =>
            aerodrome
              ? {
                  ...aerodrome,
                  runways: aerodrome.runways?.map((item) => {
                    if (item.id === savedData.id || newData.id === item.id) {
                      return {
                        ...
                        intersection_departure_length_ft:
                          savedData.intersection_departure_length_ft,
