...
import apiClient, {
  EditWaypointData,
  WaypointDataFromAPI,
} from '../../services/userWaypointClient';
...
const useEditUserWaypoint = () => {
  ...
  >({
    ...
    onMutate: (newData) => {
      const previusData = queryClient.getQueryData<WaypointDataFromAPI[]>([
        'waypoints',
        'user',
      ]);
      ...
      queryClient.setQueryData<WaypointDataFromAPI[]>(
        ...
        (currentData) => {
          ...
          if (newData.id !== 0) {
            const currentWaypoint = currentData?.find(
              (item) => item.id == newData.id,
            );
            newWaypointInCacheFormat.created_at_utc =
              currentWaypoint?.created_at_utc || utcNow;
            return currentData?.map((item) => {
              if (item.id === newData.id) return newWaypointInCacheFormat;
              return item;
            });
          }
